...
class CardScatter(Scatter, CardsList):
    ...
    def on_touch_down(self, touch):
        if not super(CardScatter, self).on_touch_down(touch):
            return False
    ...
    def on_touch_up(self, touch):
        if not super(CardScatter, self).on_touch_up(touch):
            return False
        if self.selected > 0:
            for child in self.images:
                child.alpha = 0
...
class Pile():
    ...
    def clear(self, base):
        ...
        if self.counter:
            if base == 0:
                self.layout.remove_widget(self.counter)
